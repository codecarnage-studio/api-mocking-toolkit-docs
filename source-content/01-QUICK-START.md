...
- ✅ 100% reproducible
 // FIXXME: Session logs? Any other features which we actually offer and can itroduce them in above scenarios? Then we can hint show in demo how we also solved in our app.
...
```mermaid
flowchart LR
    A[Your Game] -->|API Call| B{API Mocking Toolkit}
    B -->|Real Server| C[Backend Online]
    B -->|Local Server| D[Local Dev Server]
    B -->|Mock Success| E[✓ Success Response]
    B -->|Mock Error| F[✗ Error Response]
...
2. Search for "API Mocking Toolkit"
3. Click `Import`
4. Import all files
// FIXME: Should we really stick to 2 options? Or not just one? Buy from asset store and import via unity Package Manager? Isnt that the only flow? Since will be a paid app.
**Requirements:**
...
// FIXME: This diagram is even more confusing. Client should be on one side, and depending upong the envt we construct the request with different variables. Thoat doesn't show in layout of diagram.

   ```mermaid
   graph LR
       Dev[Dev] -->|baseUrl=http://localhost:3000| ApiClient
       QA[QA] -->|baseUrl=https://qa.api.mygame.com| ApiClient
       ProdEU[Prod EU] -->|baseUrl=https://eu.api.mygame.com| ApiClient
       ProdNA[Prod NA] -->|baseUrl=https://na.api.mygame.com| ApiClient
       ApiClient --> Mock[Mocked responses]
       ApiClient --> Real[Real backend]
   ```
...
**Continue learning:**
- [Guides](guides.md) – Workflows and real-world scenarios
- [API Reference](api-reference.md) – Full API surface and code examples
...
- Check [Troubleshooting FAQ](api-reference.md#troubleshooting)
